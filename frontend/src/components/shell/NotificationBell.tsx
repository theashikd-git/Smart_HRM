...
/**
 * Shared bell icon for the top-right nav -- used on BOTH the staff
 * Workbench (TopNavigation) and the Employee Portal (EmployeePortalView).
 * There's exactly one notification feed behind it: /notifications/mine is
 * scoped to whoever is signed in (their User id), staff and Employee
 * Portal accounts alike, so a leave request awaiting a staff approver's
 * decision shows up here the same way a status update shows up for the
 * employee who applied -- no separate staff/employee notification systems
 * to keep in sync (see backend NotificationsService, populated from
 * LeaveService.create/approve/reject and RosterService.upsert/remove).
 */
